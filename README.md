...
## 思路
1. 首先修改create_matrix()函数，增添初始化矩阵所有元素为0的操作。
2. 对于矩阵加法、减法、数乘，就不多说了。
3. 矩阵乘法，只要知道乘积矩阵每个元素的算法（矩阵A的第i行和矩阵B的第j列元素依次乘积求和），使用三重循环即可。
4. 矩阵的转置：对i*j的矩阵，创建一个j*i的矩阵，元素沿对角线换位即可。
5. 矩阵的行列式：使用递归是比较容易想到的思路，关键是递归的形式（Lapalce定理）和递归出口（n=1的情况）。
6. 矩阵的逆：只需求出矩阵的行列式和伴随矩阵，伴随矩阵使用公式构造即可。
7. 矩阵的迹：只要知道矩阵的迹是对角线元素之和。
8. 矩阵的秩：**比较复杂**。首先，创建原矩阵的副本（因为过程中会改变矩阵）；然后通过高斯消元法将矩阵转换为行阶梯形状（该算法通过一系列的行操作（如行交换、行相加、行数乘）把矩阵进行简化）；在转换过程中，对于矩阵的每一列，选择绝对值最大的元素（主元）进行操作，以减小计算误差（部分主元选择法）；将主元所在的行与当前操作行交换，确保每一步操作都是在绝对值最大的元素下进行；如果主元所在的行已经是当前操作行，则不用交换；使用当前选择的主元下方的所有元素进行消元，即用当前行减去其下面的行的若干倍，使得该列的下方元素都变为0；当通过上述步骤将矩阵转化为行阶梯形状后，非零行的数量（即从上至下第一个非零元素的行）将是该矩阵的秩。